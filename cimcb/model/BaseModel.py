...
from ..plot import scatter, scatterCI, boxplot, distribution, permutation_test, roc_boot, roc
...
class BaseModel(ABC):
    ...
    def evaluate(self, test=None, plot_median=False, specificity=False, cutoffscore=False, bootnum=100, title_align="left", dist_smooth=None, bootmethod='BCA'):
        ...
        if test is not None:
            Ytrue_test = np.array(test[0])
            Yscore_test = np.array(test[1])
