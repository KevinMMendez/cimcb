...
class kfold(BaseCrossVal):
    ...
    def calc_ypred_epoch(self):
        ...
        def mc_loop(j):
            ...
            k = model_i.k
            ypred = [self.ypred_full, self.ypred_cv, k]
            return ypred
        ...
        for i in self.avstat:
            ...
            # Put ypred into standard format
            epoch_list = []
            for j in self.param_list2:
                for k, v in j.items():
                    epoch_list.append(v - 1)
            for m in epoch_list:
                ypred_full_i = Y_full[m]
                ypred_cv_i = Y_cv[m]
    ...
    def calc_stats(self):
        ...
        for i in range(len(self.param_list)):
            ...
            for k in range(len(self.ypred_full)):
                full2 = binary_metrics(self.Y, self.ypred_full[k][i], parametric=self.model.parametric, k=self.k[k][i])
                cv2 = binary_metrics(self.Y, self.ypred_cv[k][i], parametric=self.model.parametric, k=self.k[k][i])
