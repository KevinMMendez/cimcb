...
class kfold(BaseCrossVal):
    ...
    def calc_stats(self):
        ...
        for i in range(len(self.param_list)):
            ...
            for j in range(len(self.ypred_full[i])):
                full = binary_metrics(self.ypred_full[i][j][0], self.ypred_full[i][j][1], parametric=self.model.parametric)
                cv = binary_metrics(self.ypred_cv[i][j][0], self.ypred_cv[i][j][1], parametric=self.model.parametric)
    ...
    def _plot_param2(self, metric="r2q2", xlabel=None, orientation=0, alternative=False, scale=1, heatmap_xaxis_rotate=90, color_scaling="linear", line=False):
        ...
        # Scatterplot
        p4 = figure(title="Scatterplot (cv - diff)", x_axis_label=cv_title, y_axis_label=diff_title, tools="tap,pan,wheel_zoom,box_zoom,reset,save,lasso_select,box_select", x_range=(min(cv_score) - 0.03, max(cv_score) + 0.03), y_range=(min(diff_score) - 0.03, max(diff_score) + 0.03))
