...
class kfold(BaseCrossVal):
    ...
    def _plot_param2(self, metric="r2q2", xlabel=None, orientation=0, alternative=False, scale=1, heatmap_xaxis_rotate=90, color_scaling="linear", line=False):
        ...
        key0_unique = np.unique(key0_value)
        key1_unique = np.unique(key1_value)
    ...
    def _plot_param3(self, metric="r2q2", xlabel=None, orientation=0, alternative=False, scale=1, heatmap_xaxis_rotate=90, color_scaling="linear", line=False):
        ...
        for key, value in param_dict.items():
            if len(value) < 2:
                raise ValueError(" There needs to be at least 2 values per parameter for a grid")
        ...
        full_score = table.loc["AUCfull"].values
        cv_score = table.loc["AUCcv"].values
        diff_score = full_score - cv_score
        ...
        # Heatmap FULL
        p1 = figure(title="AUC FULL", tools="tap, save", x_range=key0_unique, y_range=key1_unique, x_axis_label=param_keys[0], y_axis_label=param_keys[1])
        ...
        # Heatmap CV
        p2 = figure(title="AUC CV", tools="tap, save", x_range=key0_unique, y_range=key1_unique, x_axis_label=param_keys[0], y_axis_label=param_keys[1])
        ...
        # Heatmap Diff
        p3 = figure(title="AUC DIFFERENCE", tools="tap, save", x_range=key0_unique, y_range=key1_unique, x_axis_label=param_keys[0], y_axis_label=param_keys[1])
        ...
        # Scatterplot
        p4 = figure(title="Scatterplot (AUC CV - AUC DIFF)", x_axis_label="AUC CV", y_axis_label="AUC DIFFERENCE", tools="tap,pan,wheel_zoom,box_zoom,reset,save,lasso_select,box_select", x_range=(min(cv_score) - 0.03, max(cv_score) + 0.03), y_range=(min(diff_score) - 0.03, max(diff_score) + 0.03))
