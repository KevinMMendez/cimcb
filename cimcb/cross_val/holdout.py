...
class holdout(BaseCrossVal):
    ...
    def __init__(self, model, X, Y, param_dict, test_size=0.8, random_state=None, stratify=True, folds=None):
    ...
    def calc_ypred(self):
        ...
        for params in tqdm(range(len(self.param_list)), desc="Holdout"):
            ...
            self.ypred_full.append([[Y_train, ypred_full_i]])
            self.ypred_cv.append([[Y_test, ypred_cv_i]])
    ...
    def calc_ypred_epoch(self):
        ...
        for i in epoch_list:
            ...
            ypred_full_i.append([Y_full[0][0], Y_full[0][1][i]])
            ypred_cv_i.append([Y_cv[0][0], Y_cv[0][1][i]])
            # Append ypred to full/cv
    ...
    def calc_stats(self):
        ...
        for i in range(len(self.param_list)):
            ...
            stats_full_i = full_loop
            stats_cv_i = cv_loop

            # Rename columns
            stats_full_i = {k + "full": v for k, v in stats_full_i[0].items()}
            stats_cv_i = {k + "cv": v for k, v in stats_cv_i[0].items()}
