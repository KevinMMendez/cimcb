...
class holdout(BaseCrossVal):
    ...
    def __init__(self, model, X, Y, param_dict, test_size=0.2, random_state=None, stratify=True, folds=None, n_cores=-1, mc=0):
        for key, value in param_dict.items():
            if not isinstance(value, (list, tuple, np.ndarray)):
                param_dict[key] = [value]

        super().__init__(model=model, X=X, Y=Y, param_dict=param_dict)
    ...
    def calc_ypred(self):
        ...
        def processInput(i):
            ...
            ypred_full_i = model_i.train(self.X_train, self.Y_train)
            ypred_cv_i = model_i.test(self.X_test)
            # Get ypred full cv
            ypred_full = [[self.Y_train, ypred_full_i]]
            ypred_cv = [[self.Y_test, ypred_cv_i]]
            k = model_i.k
            ypredcvfull = [ypred_full, ypred_cv, k]
            return ypredcvfull

        def mc_loop(j):
            self.X_train, self.X_test, self.Y_train, self.Y_test = train_test_split(self.X, self.Y, test_size=self.test_size, random_state=None, stratify=self.stratify)
    ...
    def calc_ypred_epoch(self):
        ...
        for k in range(len(Y_full)):
            ...
            for i in epoch_list:
                ...
                ypred_full_i.append([Y_full[k][0][0], Y_full[k][0][1][i]])
                ypred_cv_i.append([Y_cv[k][0][0], Y_cv[k][0][1][i]])
        ...
        for i in range(len(self.param_list)):
            ...
            for k in range(len(self.ypred_full)):
                for j in range(len(self.ypred_full[k][i])):
                    full2 = binary_metrics(self.ypred_full[k][i][j][0], self.ypred_full[k][i][j][1], parametric=self.model.parametric, k=self.k[k][i])
                    cv2 = binary_metrics(self.ypred_cv[k][i][j][0], self.ypred_cv[k][i][j][1], parametric=self.model.parametric, k=self.k[k][i])
                    full_loop.append(full2)
                    cv_loop.append(cv2)
